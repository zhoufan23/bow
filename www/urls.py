...
def index():
    ...
    return dict(blogs=blogs, page=page, user=ctx.request.user)
...
def authenticate():
    ...
    max_age = 604800 if i.remember == 'true' else None
...
def api_get_blog_comment(blog_id):
    count = Comment.count_by('where blog_id=?', blog_id)
    page = Page(count, _get_page_index())
