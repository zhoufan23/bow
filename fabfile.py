...
def backup():
    ...
    with cd('/tmp'):
        run(
            'mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick bow > %s' % (
...
def restore2local():
    ...
    sqls = [
        ...
        'create database bow;',
        'grant select, insert, update, delete on bow.* to \'%s\'@\'localhost\' identified by \'%s\';' % (
        db_user, db_password)
    ]
    for sql in sqls:
