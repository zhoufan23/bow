...
env.user = 'ubuntu'
env.sudo_user = 'root'
env.hosts = ['18.188.176.252']

db_user = 'root'
db_password = ''

_TAR_FILE = 'dist-bow.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '~/bow'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-bow-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick bow > %s' % (db_user, db_password, f))
...
def restore2local():
    ...
    sqls = [
        ...
        'create database bow;',
        'grant select, insert, update, delete on bow.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s bow < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
